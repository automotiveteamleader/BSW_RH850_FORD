...
{
  {
    ...
    for(idx = (SpinlockIdType)0; idx < Os_SpinlockGetSpinlockIdCount(); ++idx)
    {
      P2CONST(Os_SpinlockConfigType, AUTOMATIC, OS_CONST) spinlock;
      P2VAR(Os_SpinlockType, AUTOMATIC, OS_VAR_NOINIT) spinlockDyn;
...
)
{
  ...
  if(OS_UNLIKELY(Spinlock->Checks == OS_SPINLOCKCHECK_ENABLED))
  {
    ...
    else if(OS_UNLIKELY(Os_LockListCheckTopIsSpinlock(locks) == OS_CHECK_FAILED))                                       /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* SBSW_OS_SL_LOCKLISTCHECKTOPISSPINLOCK_001 */
    {
    ...
    else if(OS_UNLIKELY(Os_AppCheckAccess(currentApplication,                                                           /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* SBSW_OS_SL_APPCHECKACCESS_001 */ /* SBSW_OS_FC_PRECONDITION */
                                          Os_SpinlockGetAccessingApplications(Spinlock)) == OS_CHECK_FAILED))
    {
      status = OS_STATUS_ACCESSRIGHTS_1;
    }
