...
))
{
  ...
  if(OS_UNLIKELY(Os_ThreadIsTask(CurrentThread) == 0))                                                                  /* SBSW_OS_FC_PRECONDITION */
  {
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreCheckIsLocalCore(me->Core, localCore) == OS_CHECK_FAILED))                                    /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* SBSW_OS_RES_CORECHECKISLOCALCORE_001 */
    {
  ...
  else
  {
    ...
    else
    {
      ...
      if(Os_ResourceIsIntResource(me) != 0)                                                                             /* SBSW_OS_RES_RESOURCEISINTRESOURCE_001 */
      {
        ...
        Os_InterruptRestore(&previousInterruptState);                                                                   /* SBSW_OS_FC_POINTER2LOCAL */ /* PRQA S 3354 */ /* MD_Os_3354_Local */
      }
