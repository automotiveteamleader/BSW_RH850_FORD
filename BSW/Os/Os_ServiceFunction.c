...
)
{
  ...
  else
  {
    ...
    if(hasTpDelay != 0)                                                                                                 /* COV_OS_HALPLATFORMTPUNSUPPORTED_XF */
    {
    ...
    /* #70 Trusted function specific call behavior. */
    {
      ...
      if(Os_ServiceTfNeedsModeSwitch(TrustedFunction) != 0)                                                             /* SBSW_OS_FC_PRECONDITION */
      {
        ...
        Os_ThreadCallContext(currentThread, TrustedFunction->Context, &serviceCall, &userInterruptState,                /* SBSW_OS_SF_THREADCALLCONTEXT_001 */
  ...
  else
  {
    ...
    else
    {
      ...
      Os_ThreadCallContextOnStack(currentThread, NonTrustedFunction->Context, ntfCall.Stack->HwConfig,                  /* SBSW_OS_SF_THREADCALLCONTEXTONSTACK_001 */
                                  &(ntfCall), &userInterruptState, &(ntfCall.Base.CallerContext),
