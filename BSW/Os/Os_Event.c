...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(OS_LIKELY(Os_TaskIsCoreLocal(task) != 0))                                                                      /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* SBSW_OS_EV_TASKISCORELOCAL_001 */
      {
        ...
        {
          if(Os_ThreadIsTask(currentThread) != 0)                                                                       /* SBSW_OS_EV_THREADISTASK_001 */
          {
            ...
            if(Os_SchedulerTaskSwitchIsNeeded(scheduler) != 0)                                                          /* SBSW_OS_EV_SCHEDULERTASKSWITCHISNEEDED_001 */
            {
  ...
  return status;
}                                                                                                                       /* PRQA S 6030, 6050, 6080 */ /* MD_Os_STCYC, MD_Os_STMIF, MD_Os_STCAL */
...
)
{
  ...
  else
  {
    ...
    else if(OS_UNLIKELY(Os_ThreadCheckSpinlocksReleased(currentThread) == OS_CHECK_FAILED))                             /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* SBSW_OS_EV_THREADCHECKSPINLOCKSRELEASED_001 */
    {
    ...
    else
    {
      ...
      if((Os_EventWaitEventTriggered(event) != 0))                                                                      /* SBSW_OS_EV_EVENTWAITEVENTTRIGGERED_001 */
      {
...
)
{
  ...
  else
  {
    ...
    if( (Os_TaskIsWaiting(Task) != 0) && (waitEventsTriggerd != 0) )                                                    /* SBSW_OS_FC_PRECONDITION */
    {
...
)
{
  ...
  currentThread = Os_CoreGetThread();
  targetCore = Os_TaskGetCore(Task);                                                                                    /* SBSW_OS_FC_PRECONDITION */
  callerCore = Os_ThreadGetCore(currentThread);                                                                         /* SBSW_OS_EV_THREADGETCORE_001 */

  /* #10 If the given task belongs to the local core, */
  if(OS_LIKELY(callerCore == targetCore))                                                                               /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
  {
  ...
  else
  {
    ...
    status = Os_XSigSend_SetEventAsync(targetCore, currentThread, Os_TaskGetId(Task), Mask);                            /* SBSW_OS_EV_XSIGSEND_001 */ /* SBSW_OS_FC_PRECONDITION */
  }
...
)
{
  ...
  else
  {
    ...
    }while((*Mask) != Os_EventTriggerGet(event));                                                                       /* COV_OS_INVSTATE */ /* SBSW_OS_EV_EVENTTRIGGERGET_001 */
